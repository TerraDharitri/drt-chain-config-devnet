...
TAG_FROM_BINARY=tag/main
#check if the tag binaryVersion file exits on the github
TAG_EXITS=$(git ls-remote https://github.com/TerraDharitri/drt-go-chain.git "${TAG_FROM_BINARY}")
if [ -z "${TAG_EXITS}" ]; then
...
if [ ! -f "${OUTPUT_FOLDER}/validatorKey.pem" ]; then
  ...
  docker run --rm --mount type=bind,source=${OUTPUT_FOLDER},destination=/keys --workdir /keys terradharitri/chain-keygenerator:latest
  if [ $? -ne 0 ]; then
...
check_nonce_grows() {
  ...
  do
    ...
    CURRENT_NONCE=$(curl -s ${ADDRESS_WITH_ROUTE} | jq '.data["status"]["drt_nonce"]')
